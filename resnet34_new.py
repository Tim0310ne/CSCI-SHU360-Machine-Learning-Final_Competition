# -*- coding: utf-8 -*-
"""
Urban Sound Classification with ResNet-34
Clean implementation based on resnet18.py structure

Key features:
- ResNet-34 architecture (34 layers, [3,4,6,3] blocks)
- SE attention blocks
...
class ResNet34(nn.Module):
    ...
    Architecture: [3, 4, 6, 3] blocks = 34 layers
    Channels: 64 -> 128 -> 256 -> 512
    
    Compared to ResNet-18:
    - More blocks: [3,4,6,3] vs [2,2,2,2]
    - More parameters: ~21M vs ~11M
    ...
    def __init__(self, in_channels=3, num_classes=NUM_CLASSES, use_se=True):
        ...
        # Residual layers: [3, 4, 6, 3] blocks (ResNet-34)
...
def train_model(model, train_loader, val_loader, epochs=50, lr=1e-3, 
                label_smoothing=0.1, use_mixup=True, save_path="resnet34_best.pth"):
    """Full training loop."""
...
def save_submission(predictions, path="resnet34.csv"):
...
def run(
    ...
    print("=" * 60)
    print("ResNet-34 Audio Classification")
    print("=" * 60)
    ...
    # Model
    model = ResNet34(in_channels=in_channels, num_classes=NUM_CLASSES, use_se=use_se)
    ...
    if TEST_CSV.exists():
        print("\nRunning inference...")
        model.load_state_dict(torch.load("resnet34_best.pth", map_location=DEVICE, weights_only=True))
        model = model.to(DEVICE)
        ...
        save_submission(predictions, "resnet34.csv")
...
def run_cv_ensemble(
    ...
    print("=" * 60)
    print("ResNet-34 - 8-Fold CV Ensemble")
    print("=" * 60)
    ...
    for val_fold in range(1, 9):
        ...
        save_path = f"resnet34_fold{val_fold}.pth"
    ...
    if TEST_CSV.exists():
        ...
        # Save submission
        save_submission(predictions, "resnet34_ensemble.csv")
